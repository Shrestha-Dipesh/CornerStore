...
if (isset($_POST['change_button'])) {
    if (isset($_POST['user_name']) && isset($_POST['email']) && isset($_POST['address']) && isset($_POST['contact']) && isset($_POST['user_date'])) {
        ...
        if (!empty($_POST['old_password']) && !empty($_POST['new_password']) && !empty($_POST['confirm_password'])) {
            ...
            if (sha1($_POST['old_password']) == $data['PASSWORD']) {
                ...
                if ($uppercase && $lowercase && $number && strlen($_POST['new_password']) >= 8) {
                    if ($_POST['new_password'] == $_POST['confirm_password']) {
                        ...
                        $sql1 = "UPDATE cornerstore.users SET user_name = '$_POST[user_name]', email = '$_POST[email]', password = '$password', address = '$_POST[address]', contact = $_POST[contact], user_date = TO_DATE('$_POST[user_date]', 'YYYY-MM-DD') WHERE user_id = $user_id";
                        $result1 = oci_parse($conn, $sql1);
                        oci_execute($result1);
                        $_SESSION['profile_successful'] = 'Changes Saved';
                        $_SESSION['name'] = $_POST['user_name'];
                        setcookie('name', $_POST['user_name'], time() + 86400);
                    } else {
            ...
            $sql2 = "UPDATE cornerstore.users SET user_name = '$_POST[user_name]', email = '$_POST[email]', address = '$_POST[address]', contact = $_POST[contact], user_date = TO_DATE('$_POST[user_date]', 'YYYY-MM-DD') WHERE user_id = $user_id";
...
header("location: trader_profile.php?id=$_POST[user_id]");
